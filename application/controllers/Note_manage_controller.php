...
class Note_manage_controller extends IMS_controller
{
	...
	{
		$this->load->model('M_note', 'mn');
		$this->mn->note_id = $this->input->post("note_id_edit");
		$this->mn->note_to_do_list = $this->input->post("note_edit");
		$this->mn->note_create_date = $this->input->post("create_note_edit");
		$this->mn->note_type = $this->input->post("note_type");
		$this->mn->update();
